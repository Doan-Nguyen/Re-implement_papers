...
import sys
sys.path.insert(1, '/home/ubuntu/Doannn/pixel_link_final/pixel_link')
import config
...
if __name__ == "__main__":
    root_dir = util.io.get_absolute_path('./dataset_ocr_raw/')
    output_dir = util.io.get_absolute_path('./dataset_ocr/')
    ...
    cvt_to_tfrecords(output_path = util.io.join_path(output_dir, 'dataset_ocr_train.tfrecord'), data_path = training_data_dir, gt_path = training_gt_dir)

    test_data_dir = util.io.join_path(root_dir, 'test_images')
    test_gt_dir = util.io.join_path(root_dir,'test_labels_gt')
    cvt_to_tfrecords(output_path = util.io.join_path(output_dir, 'dataset_ocr_test.tfrecord'), data_path = test_data_dir, gt_path = test_gt_dir)
